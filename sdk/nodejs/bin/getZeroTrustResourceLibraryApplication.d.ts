import * as pulumi from "@pulumi/pulumi";
/**
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as cloudflare from "@pulumi/cloudflare";
 *
 * const exampleZeroTrustResourceLibraryApplication = cloudflare.getZeroTrustResourceLibraryApplication({
 *     accountId: "023e105f4ecef8ad9ca31a8372d0c353",
 *     id: "0b63249c-95bf-4cc0-a7cc-d7faaaf1dac0",
 * });
 * ```
 */
export declare function getZeroTrustResourceLibraryApplication(args: GetZeroTrustResourceLibraryApplicationArgs, opts?: pulumi.InvokeOptions): Promise<GetZeroTrustResourceLibraryApplicationResult>;
/**
 * A collection of arguments for invoking getZeroTrustResourceLibraryApplication.
 */
export interface GetZeroTrustResourceLibraryApplicationArgs {
    accountId: string;
    /**
     * The ID of this resource.
     */
    id: string;
}
/**
 * A collection of values returned by getZeroTrustResourceLibraryApplication.
 */
export interface GetZeroTrustResourceLibraryApplicationResult {
    readonly accountId: string;
    /**
     * Confidence score for the application. Returns -1 when no score is available.
     */
    readonly applicationConfidenceScore: number;
    /**
     * Returns the score composition breakdown for the application.
     */
    readonly applicationScoreComposition: string;
    /**
     * Returns the application source.
     */
    readonly applicationSource: string;
    /**
     * Returns the application type.
     */
    readonly applicationType: string;
    /**
     * Returns the application type description.
     */
    readonly applicationTypeDescription: string;
    /**
     * Returns the application creation time.
     */
    readonly createdAt: string;
    /**
     * GenAI score for the application. Returns -1 when no score is available.
     */
    readonly genAiScore: number;
    /**
     * Returns the list of hostnames for the application.
     */
    readonly hostnames: string[];
    /**
     * Returns the human readable ID.
     */
    readonly humanId: string;
    /**
     * The ID of this resource.
     */
    readonly id: string;
    /**
     * Returns the Intel API ID for the application.
     */
    readonly intelId: number;
    /**
     * Returns the list of IP subnets for the application.
     */
    readonly ipSubnets: string[];
    /**
     * Returns the application name.
     */
    readonly name: string;
    /**
     * Returns the list of port protocols for the application.
     */
    readonly portProtocols: string[];
    /**
     * Returns the list of support domains for the application.
     */
    readonly supportDomains: string[];
    /**
     * Cloudflare products that support this application.
     */
    readonly supporteds: string[];
    /**
     * Returns the application update time.
     */
    readonly updatedAt: string;
    /**
     * Returns the application version.
     */
    readonly version: string;
}
/**
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as cloudflare from "@pulumi/cloudflare";
 *
 * const exampleZeroTrustResourceLibraryApplication = cloudflare.getZeroTrustResourceLibraryApplication({
 *     accountId: "023e105f4ecef8ad9ca31a8372d0c353",
 *     id: "0b63249c-95bf-4cc0-a7cc-d7faaaf1dac0",
 * });
 * ```
 */
export declare function getZeroTrustResourceLibraryApplicationOutput(args: GetZeroTrustResourceLibraryApplicationOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetZeroTrustResourceLibraryApplicationResult>;
/**
 * A collection of arguments for invoking getZeroTrustResourceLibraryApplication.
 */
export interface GetZeroTrustResourceLibraryApplicationOutputArgs {
    accountId: pulumi.Input<string>;
    /**
     * The ID of this resource.
     */
    id: pulumi.Input<string>;
}
//# sourceMappingURL=getZeroTrustResourceLibraryApplication.d.ts.map